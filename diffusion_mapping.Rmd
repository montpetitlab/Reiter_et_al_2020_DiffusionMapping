...
dc10_19p <-ggplot(dc19, aes(x = DC10, y = 0, color = brix, label = ava_id)) +
  ...
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.y = element_blank(),
        axis.title.y = element_blank()) 
all_dc_19 <- ggarrange(dc2_19p, dc3_19p, dc4_19p, dc5_19p, dc6_19p,
                       dc7_19p, dc8_19p, ncol = 1, legend = NULL,
...
```{r DC2DC42019, fig.height= 4, fig.cap = "One-dimensional plots of diffusion components 2-4 for the 2019 vintage colored by **A** AVA and **B** hour. Diffusion components capture different latent structure in the dataset. Each component represents a different relationship among samples."}
...
```{r DC5DC82019, fig.height= 4, fig.cap = "One-dimensional plots of diffusion components 1-10 for the 2019 vintage colored by **A** AVA and **B** hour. Diffusion components capture different latent structure in the dataset. Each component represents a different relationship among samples."}
...
all_dc_17 <- ggarrange(dc2_17p, dc3_17p, dc4_17p, legend = "none",
...
all_dc_17_ava <- ggarrange(dc2_17p, dc3_17p, dc4_17p, legend = "none",
...
Y <- dc_y19$E[gene, ]
dc19$Y = Y
ggplot(dc19, aes(y = Y, x = DC2, color = ava)) +
  geom_smooth(method='lm', color = "grey", se = F, alpha = .5) +
  geom_point(alpha = .9) +
  ggtitle("ARG1") +
  theme_minimal() +
...
```{r DC1hypoxiaWT, fig.height = 2, fig.cap="Diffusion map of wildtype and 22 mutant strains exposed to 100% nitrogen for 0-240 minutes. Wildtype cells are triangles, while mutants appear as points."}
# "#E8ECFB", "#521A13"
