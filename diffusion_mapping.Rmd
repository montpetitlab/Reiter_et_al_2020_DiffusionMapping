...
dc_top_enrich_plot <- function(dc, dc_num, dc_con, color_by = c("brix", "ava", "hr"), show_genes = F){
  ...
  if(plot_by == "ava"){
    plt_dc <- ggplot(dc, aes(x = DC, y = ava_id, color=hr)) +
      geom_vline(xintercept = mean(dc$DC) + 2 * sd(dc$DC), linetype = "dotted") +
      geom_vline(xintercept = -mean(dc$DC) - 2 * sd(dc$DC), linetype = "dotted") +  
      geom_vline(xintercept = mean(dc$DC) + sd(dc$DC), linetype = "dashed") +
      geom_vline(xintercept = -mean(dc$DC) - sd(dc$DC), linetype = "dashed") + 
      geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
      scale_color_brewer(palette = "Paired", name = "time (hour)")  +
      theme_minimal() +
      theme(legend.position = "none") +  
      scale_y_discrete(limits=rev) +
      labs(color = expression(degree*Brix), y = "site", x = dc_num) 
  } else if(plot_by == "hr"){
    plt_dc <- ggplot(dc, aes(x = DC, y = hr, color=brix)) +
      geom_vline(xintercept = mean(dc$DC) + 2 * sd(dc$DC), linetype = "dotted") +
      geom_vline(xintercept = -mean(dc$DC) - 2 * sd(dc$DC), linetype = "dotted") +  
      geom_vline(xintercept = mean(dc$DC) + sd(dc$DC), linetype = "dashed") +
      geom_vline(xintercept = -mean(dc$DC) - sd(dc$DC), linetype = "dashed") +  
      ...
      theme(legend.position = "none") +  
      scale_y_discrete(limits=rev) +
      labs(color = expression(degree*Brix), y = "time (hours)", x = dc_num) 
  } else {
...
dc_top_diffex_plot <- function(dc, dc_num, dc_con, plot_by = c("ava", "hr")){
  # dc <- dc19
  # dc_num <- "DC2"
  # dc_con <- dc2_19
  # plot_by = "ava"
...
brix_plot <- ggplot(brix19, aes(x = hours_post_innoculation, y = brix, color = ava_id, shape = tank)) +
  geom_line(alpha = .7) +
  labs(x = "time (hours)", y = expression(degree*Brix), color = "site", shape = "tank") +
  theme_minimal() +
...
#all.equal(brix_19_all$ORF, dc1_19_all$ORF)

all_l2fc <- data.frame(brix19 = brix_19_all$logFC_adj * -1,
                       DC1_19 = dc1_19_all$logFC_adj) 

lm_plt <- ggplot(all_l2fc, aes(x = brix19, y = DC1_19)) +
  geom_hex() +
  theme_minimal() +
  scale_fill_continuous(high = "#3D3D3D", low = "#EBECF0") +
  labs(x = expression(log[2]~fold~change~degree*Brix),
       y = expression(log[2]~fold~change~DC1))
```

```{r dc1}
#library(raincloudplots)
library(ggbeeswarm2)
dc19$hr <- factor(dc19$hr, levels = c(2, 6, 16, 64, 112))
p19 <- ggplot(dc19, aes(x = DC1, y = hr, color=brix)) +
  geom_beeswarm(size=2, spacing =1, alpha = .5, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
  theme_minimal() +
  scale_y_discrete(limits=rev) +
  labs(color = expression(degree*Brix), y = "time (hours)") 


# p19 <- ggplot(dc19, aes(y = DC1, x = hr, color=brix)) +
#   #geom_point(alpha = .5, size = 5) +
#   scale_color_viridis_c() +
#   theme_minimal() +
#   labs(color = expression(degree*Brix), x = "hour") +
#   #geom_flat_violin(position = position_nudge(x = .3, y = 0), alpha = 1, fill = "black") +
#   #geom_point(aes(x = hr, color = brix), position = position_jitter(width = .15), size = 2, alpha = 0.7) +
#   geom_flat_violin(position = position_nudge(x = .15, y = 0), alpha = 1, fill = "black") +
#   geom_point(aes(x = hr, color = brix), size = 2, alpha = 0.7) +
#   scale_x_discrete(limits=rev)+
#   coord_flip() 
ggarrange(p19, lm_plt, ncol = 2, legend = "bottom", labels = c(" ", " "), widths = c(2, 1))
```
...
kable(num_diffex, "latex", booktabs = T, col.names = c("diffusion component", "induced", "repressed", "induced", "repressed"),
      caption = 'Number of significantly differentially expressed genes for the top diffusion components.') %>%
  kable_styling(font_size = 9) 
  #add_header_above(c(" " = 1, "2017" = 2, "2019" = 2), align = "l", escape = F)
```

```{r all_dc19_hour}
dc2_19p <- ggplot(dc19, aes(x = DC2, y = hr, color=brix)) +
  geom_vline(xintercept = mean(dc19$DC2) + 2 * sd(dc19$DC2), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC2) - 2 * sd(dc19$DC2), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC2) + sd(dc19$DC2), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC2) - sd(dc19$DC2), linetype = "dashed") + 
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
  xlim(c(-0.2, .2)) +
...
dc3_19p <- ggplot(dc19, aes(x = DC3, y = hr, color=brix)) +
      geom_vline(xintercept = mean(dc19$DC3) + 2 * sd(dc19$DC3), linetype = "dotted") +
      geom_vline(xintercept = -mean(dc19$DC3) - 2 * sd(dc19$DC3), linetype = "dotted") +  
      geom_vline(xintercept = mean(dc19$DC3) + sd(dc19$DC3), linetype = "dashed") +
      geom_vline(xintercept = -mean(dc19$DC3) - sd(dc19$DC3), linetype = "dashed") + 
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  xlim(c(-0.2, .2)) +
  scale_color_viridis_c() +
...
dc4_19p <- ggplot(dc19, aes(x = DC4, y = hr, color=brix)) +
      geom_vline(xintercept = mean(dc19$DC4) + 2 * sd(dc19$DC4), linetype = "dotted") +
      geom_vline(xintercept = -mean(dc19$DC4) - 2 * sd(dc19$DC4), linetype = "dotted") +  
      geom_vline(xintercept = mean(dc19$DC4) + sd(dc19$DC4), linetype = "dashed") +
      geom_vline(xintercept = -mean(dc19$DC4) - sd(dc19$DC4), linetype = "dashed") + 
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
  xlim(c(-0.2, .2)) +
...
dc5_19p <- ggplot(dc19, aes(x = DC5, y = hr, color=brix)) +
  geom_vline(xintercept = mean(dc19$DC5) + 2 * sd(dc19$DC5), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC5) - 2 * sd(dc19$DC5), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC5) + sd(dc19$DC5), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC5) - sd(dc19$DC5), linetype = "dashed") + 
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
  xlim(c(-0.4, 0.4)) +
...
dc6_19p <- ggplot(dc19, aes(x = DC6, y = hr, color=brix)) +
  geom_vline(xintercept = mean(dc19$DC6) + 2 * sd(dc19$DC6), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC6) - 2 * sd(dc19$DC6), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC6) + sd(dc19$DC6), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC6) - sd(dc19$DC6), linetype = "dashed") +  
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
  xlim(c(-0.4, 0.4)) +
...
dc7_19p <- ggplot(dc19, aes(x = DC7, y = hr, color=brix)) +
  geom_vline(xintercept = mean(dc19$DC7) + 2 * sd(dc19$DC7), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC7) - 2 * sd(dc19$DC7), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC7) + sd(dc19$DC7), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC7) - sd(dc19$DC7), linetype = "dashed") +  
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
  xlim(c(-0.4, 0.4)) +
...
dc8_19p <- ggplot(dc19, aes(x = DC8, y = hr, color=brix)) +
  geom_vline(xintercept = mean(dc19$DC8) + 2 * sd(dc19$DC8), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC8) - 2 * sd(dc19$DC8), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC8) + sd(dc19$DC8), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC8) - sd(dc19$DC8), linetype = "dashed") +  
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
  xlim(c(-0.4, 0.4)) +
...
```{r all_dc19_ava}
dc2_19p_ava <- ggplot(dc19, aes(x = DC2, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(dc19$DC2) + 2 * sd(dc19$DC2), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC2) - 2 * sd(dc19$DC2), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC2) + sd(dc19$DC2), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC2) - sd(dc19$DC2), linetype = "dashed") + 
  ...
  theme_minimal() +
  scale_y_discrete(limits=rev) +
  labs(color = expression(degree*Brix), y = "site") 

dc3_19p_ava <- ggplot(dc19, aes(x = DC3, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(dc19$DC3) + 2 * sd(dc19$DC3), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC3) - 2 * sd(dc19$DC3), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC3) + sd(dc19$DC3), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC3) - sd(dc19$DC3), linetype = "dashed") + 
...
dc4_19p_ava <- ggplot(dc19, aes(x = DC4, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(dc19$DC4) + 2 * sd(dc19$DC4), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC4) - 2 * sd(dc19$DC4), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC4) + sd(dc19$DC4), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC4) - sd(dc19$DC4), linetype = "dashed") + 
...
dc5_19p_ava <- ggplot(dc19, aes(x = DC5, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(dc19$DC5) + 2 * sd(dc19$DC5), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC5) - 2 * sd(dc19$DC5), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC5) + sd(dc19$DC5), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC5) - sd(dc19$DC5), linetype = "dashed") + 
...
dc6_19p_ava <- ggplot(dc19, aes(x = DC6, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(dc19$DC6) + 2 * sd(dc19$DC6), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC6) - 2 * sd(dc19$DC6), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC6) + sd(dc19$DC6), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC6) - sd(dc19$DC6), linetype = "dashed") + 
...
dc7_19p_ava <- ggplot(dc19, aes(x = DC7, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(dc19$DC7) + 2 * sd(dc19$DC7), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC7) - 2 * sd(dc19$DC7), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC7) + sd(dc19$DC7), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC7) - sd(dc19$DC7), linetype = "dashed") + 
...
dc8_19p_ava <- ggplot(dc19, aes(x = DC8, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(dc19$DC8) + 2 * sd(dc19$DC8), linetype = "dotted") +
  geom_vline(xintercept = -mean(dc19$DC8) - 2 * sd(dc19$DC8), linetype = "dotted") +  
  geom_vline(xintercept = mean(dc19$DC8) + sd(dc19$DC8), linetype = "dashed") +
  geom_vline(xintercept = -mean(dc19$DC8) - sd(dc19$DC8), linetype = "dashed") + 
...
ggarrange(dc2_dc4_19_hr, dc2_dc4_19_ava, legend = "none")
```
...
dc5_dc8_19_hr <- ggarrange(dc5_19p, dc6_19p, dc7_19p, dc8_19p, ncol = 1, 
                           ...
                           labels = c("A", "", "", ""))
pdf("DC3-DC8_revision.pdf", width = 6.5, height = 8.5)
ggarrange(dc5_dc8_19_hr, dc5_dc8_19_ava, legend = "none")
dev.off()
```
...
dc_top_enrich_plot_hyp <- function(dc, dc_num, dc_con, show_genes = F){
  ...
  dc$time <- factor(dc$time, levels = c(0, 5,10,30, 60, 120, 180, 240))
  plt_dc <- ggplot(dc, aes(x = DC, y = time, color = time)) +
    geom_point(alpha=.7) +
...
# "#E8ECFB", "#521A13"
dc_hyp$time <- factor(dc_hyp$time, levels = c(0, 5, 10, 30, 60, 120, 180, 240))
wt_dc1 <- ggplot(data=subset(dc_hyp, deletion == "HAP1"), 
                 aes(x = DC1, y = time, color = time)) +
...
wt_dc6 <- ggplot(data=subset(dc_hyp, deletion == "HAP1"), 
                 aes(x = DC6, y = time, color = time)) +
  geom_point(size = 3, alpha = .7) +
  labs(y = "time (minutes)") +
  scale_y_discrete(limits=rev) +
  scale_color_manual(values = c("#CE2220", "#E67F33", "#D0B541", "#7EB875", "#57A2AC", "#4E78C4", "#824D99", "#B997C7")) +
...
library(xlsx)
write.xlsx(dc1_hyp, file="supplemental_tables/supplementary_table1.xlsx", sheetName="DC1_hypoxia", row.names=FALSE)
write.xlsx(dc6_hyp, file="supplemental_tables/supplementary_table1.xlsx", sheetName="DC6_hypoxia", append = T, row.names=FALSE)
```
...
```{r enrichment_against_bendji}
hypoxic_sets <- hypoxic %>%
  dplyr::select(oxygen.regulated, gene)
tmp <- aerobic %>%
  dplyr::select(oxygen.regulated, gene)
hypoxic_sets <- rbind(hypoxic_sets, tmp)
universe <- dc1_hyp_all$ORF[!dc1_hyp_all$ORF %in% hypoxic_sets$gene] 
universe <- data.frame(oxygen.regulated = "other", gene = universe)
hypoxic_sets <- rbind(hypoxic_sets, universe)
table(hypoxic_sets$oxygen.regulated)
pathway_names <- data.frame(path = c("hypoxic", "aerobic", "other"), 
                            name = c("hypoxic", "aerobic", "other"))
dc1_hyp_all_up_enrich <- enricher(gene = dc1_hyp_all_up$ORF, 
                                  minGSSize = 10, maxGSSize = 1000,
                                  TERM2GENE = hypoxic_sets, TERM2NAME = pathway_names)
View(dc1_hyp_all_up_enrich@result)
dc1_hyp_all_down_enrich <- enricher(gene = dc1_hyp_all_down$ORF, 
                                    minGSSize = 10, maxGSSize = 1000,
                                    TERM2GENE = hypoxic_sets)
dc1_hyp_all_down_enrich@result
```
...
nitrogen$ava_id <- factor(nitrogen$ava_id, 
                          levels = c('OR1', 'OR2', 'AV1', 'AV2',  
                                     ...
                                     'CRN1', 'AS1', 'AS2', 'SMV1', 
                                     'SMV2','SRH1'))
nitrogen_plt <- ggplot(nitrogen, aes(x = ava_id, y = values, fill = ava)) +
  geom_col() +
...
ggarrange(nitrogen_plt, ggarrange(nh3_corr, nopa_corr, ncol = 2), nrow = 2, 
          common.legend = T, legend = "bottom")

# make plot with extra legend
legend_plt <- ggplot(nitrogen, aes(x = ava_id, y = values, fill = ava)) +
  geom_col() +
  facet_wrap(~variable, ncol = 2) +
  labs(x = "vineyard site", y = "concentration") +
  theme_minimal() +
  ...
  scale_fill_manual(values = c(SRH = srh, SMV = smv,  
                                AS = as, SNC = snc,  CRN = crn, 
                                RRV = rrv, AV = av, OR = or),
                    breaks = c("OR", 'AV', 'SNC',  'RRV', 
                               'CRN', 'AS', 'SMV', 'SRH'), 
                    labels = c("Willamette Valley (OR)", "Anderson Valley (AV)",
                               "Sonoma Coast (SNC)", "Russian River Valley (RRV)",
                               "Carneros (CRN)", "Arroyo Seco (AS)", 
                               "Santa Maria Valley (SMV)", "Santa Rita Hills (SRH)"),
                    name = "AVA")
```
...
```{r pca19}
library(DESeq2)
# check that colnames in counts match rownames in info
all.equal(colnames(counts19), dc19$sample)

design19 <- dc19 %>%
  dplyr::select(sample, ava_id, hr,  brix, ava)
design19$hr <- factor(design19$hr, levels = c(2, 6, 16, 64, 112))

dds19 <- DESeqDataSetFromMatrix(countData = counts19, 
                                colData = design19, 
                                design = ~ ava_id + hr)
vsd19 <-  vst(dds19)
# perform a PCA on the data in assay(x) for the selected genes
pca19 <- prcomp(t(assay(vsd19)))

# get loadings for pca
# Positive loadings indicate a variable and a principal component are positively correlated: 
# an increase in one results in an increase in the other. 
# Negative loadings indicate a negative correlation. 
# Large loadings indicate that a variable has a strong effect on that principal component.
pca19_loadings <- as.data.frame(pca19$rotation)

# the contribution to the total variance for each component
percentVar <- pca19$sdev^2 / sum(pca19$sdev^2)
...
pca_brix19 <- ggplot(pca19_df, aes(x=PC1, y=PC2, color=brix)) + 
  geom_point(size=2, alpha = .7) + 
  xlab(paste0("PC1: ",round(percentVar[1] * 100),"% variance")) +
  ylab(paste0("PC2: ",round(percentVar[2] * 100),"% variance")) +
  theme_minimal() +
  scale_color_viridis_c() +
  labs(color = expression(degree*Brix)) 

# pca_hr19 <- ggplot(pca19_df, aes(x=PC1, y=PC2, color=hr)) + 
#   geom_point() + 
#   xlab(paste0("PC1: ",round(percentVar[1] * 100),"% variance")) +
#   ylab(paste0("PC2: ",round(percentVar[2] * 100),"% variance")) +
#   theme_minimal() 
# 
# pca_ava19 <- ggplot(pca19_df, aes(x=PC1, y=PC2, color=ava)) + 
#   geom_point() + 
#   xlab(paste0("PC1: ",round(percentVar[1] * 100),"% variance")) +
#   ylab(paste0("PC2: ",round(percentVar[2] * 100),"% variance")) +
#   theme_minimal() +
...
# d0_19 <- DGEList(counts19)              # initialize the DGE object
# d19 <-calcNormFactors(d0_19)            # calculate normalization factors
# all.equal(colnames(counts19), pca19_df$sample)
pca_mm19 <- model.matrix(~pca19_df$PC1 + pca19_df$PC2 + pca19_df$PC3 + pca19_df$PC4 +
                          pca19_df$PC5 + pca19_df$PC6 + pca19_df$PC7 + pca19_df$PC8)
pca_y19 <- voom(d19, pca_mm19, plot = F)
pca_fit19 <- lmFit(pca_y19, pca_mm19)

pca1_19_all <- dc_contrast(dc_fit = pca_fit19, coeff = 2, 
                           adj.p.filt = 1, filter_logFC = F) %>%
  arrange(ORF) %>%
  mutate(logFC = logFC*-1)

pca2_19_all <- dc_contrast(dc_fit = pca_fit19, coeff = 3, 
                           adj.p.filt = 1, filter_logFC = F) %>%
  arrange(ORF)

pca3_19_all <- dc_contrast(dc_fit = pca_fit19, coeff = 4, 
                           adj.p.filt = 1, filter_logFC = F) %>%
  arrange(ORF) 

pca4_19_all <- dc_contrast(dc_fit = pca_fit19, coeff = 5, 
                           adj.p.filt = 1, filter_logFC = F) %>%
  arrange(ORF)

pca5_19_all <- dc_contrast(dc_fit = pca_fit19, coeff = 6, 
                           adj.p.filt = 1, filter_logFC = F) %>%
  arrange(ORF)

pca6_19_all <- dc_contrast(dc_fit = pca_fit19, coeff = 7, 
                           adj.p.filt = 1, filter_logFC = F) %>%
  arrange(ORF)

pca7_19_all <- dc_contrast(dc_fit = pca_fit19, coeff = 8, 
                           adj.p.filt = 1, filter_logFC = F) %>%
  arrange(ORF)

pca8_19_all <- dc_contrast(dc_fit = pca_fit19, coeff = 9, 
                           adj.p.filt = 1, filter_logFC = F) %>%
  arrange(ORF)
...
# all.equal(brix_19_all$ORF, dc1_19_all$ORF, pca_19_all$ORF)

all_l2fc <- data.frame(brix = brix_19_all$logFC_adj * -1,
                       DC1 = dc1_19_all$logFC_adj,
                       DC2 = DC2_19_all$logFC_adj,
                       DC3 = DC3_19_all$logFC_adj,
                       DC4 = DC4_19_all$logFC_adj,
                       DC5 = DC5_19_all$logFC_adj,
                       DC6 = DC6_19_all$logFC_adj,
                       DC7 = DC7_19_all$logFC_adj,
                       DC8 = DC8_19_all$logFC_adj,
                       pca1 = pca1_19_all$logFC_adj,
                       pca2 = pca2_19_all$logFC_adj, 
                       pca3 = pca3_19_all$logFC_adj, 
                       pca4 = pca4_19_all$logFC_adj,
                       pca5 = pca5_19_all$logFC_adj,
                       pca6 = pca6_19_all$logFC_adj, 
                       pca7 = pca7_19_all$logFC_adj, 
                       pca8 = pca8_19_all$logFC_adj) 
...
```{r all_pca19_hour}
pca1_19p <- ggplot(pca19_df, aes(x = PC1, y = hr, color=brix)) +
  geom_vline(xintercept = mean(pca19_df$PC1) + 2 * sd(pca19_df$PC1), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC1) - 2 * sd(pca19_df$PC1), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC1) + sd(pca19_df$PC1), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC1) - sd(pca19_df$PC1), linetype = "dashed") + 
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
  theme_minimal() +
...
pca2_19p <- ggplot(pca19_df, aes(x = PC2, y = hr, color=brix)) +
  geom_vline(xintercept = mean(pca19_df$PC2) + 2 * sd(pca19_df$PC2), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC2) - 2 * sd(pca19_df$PC2), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC2) + sd(pca19_df$PC2), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC2) - sd(pca19_df$PC2), linetype = "dashed") + 
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
...
pca3_19p <- ggplot(pca19_df, aes(x = PC3, y = hr, color=brix)) +
      geom_vline(xintercept = mean(pca19_df$PC3) + 2 * sd(pca19_df$PC3), linetype = "dotted") +
      geom_vline(xintercept = -mean(pca19_df$PC3) - 2 * sd(pca19_df$PC3), linetype = "dotted") +  
      geom_vline(xintercept = mean(pca19_df$PC3) + sd(pca19_df$PC3), linetype = "dashed") +
      geom_vline(xintercept = -mean(pca19_df$PC3) - sd(pca19_df$PC3), linetype = "dashed") + 
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
...
pca4_19p <- ggplot(pca19_df, aes(x = PC4, y = hr, color=brix)) +
      geom_vline(xintercept = mean(pca19_df$PC4) + 2 * sd(pca19_df$PC4), linetype = "dotted") +
      geom_vline(xintercept = -mean(pca19_df$PC4) - 2 * sd(pca19_df$PC4), linetype = "dotted") +  
      geom_vline(xintercept = mean(pca19_df$PC4) + sd(pca19_df$PC4), linetype = "dashed") +
      geom_vline(xintercept = -mean(pca19_df$PC4) - sd(pca19_df$PC4), linetype = "dashed") + 
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
...
pca5_19p <- ggplot(pca19_df, aes(x = PC5, y = hr, color=brix)) +
  geom_vline(xintercept = mean(pca19_df$PC5) + 2 * sd(pca19_df$PC5), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC5) - 2 * sd(pca19_df$PC5), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC5) + sd(pca19_df$PC5), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC5) - sd(pca19_df$PC5), linetype = "dashed") + 
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
...
pca6_19p <- ggplot(pca19_df, aes(x = PC6, y = hr, color=brix)) +
  geom_vline(xintercept = mean(pca19_df$PC6) + 2 * sd(pca19_df$PC6), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC6) - 2 * sd(pca19_df$PC6), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC6) + sd(pca19_df$PC6), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC6) - sd(pca19_df$PC6), linetype = "dashed") +  
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
...
pca7_19p <- ggplot(pca19_df, aes(x = PC7, y = hr, color=brix)) +
  geom_vline(xintercept = mean(pca19_df$PC7) + 2 * sd(pca19_df$PC7), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC7) - 2 * sd(pca19_df$PC7), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC7) + sd(pca19_df$PC7), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC7) - sd(pca19_df$PC7), linetype = "dashed") +  
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
...
pca8_19p <- ggplot(pca19_df, aes(x = PC8, y = hr, color=brix)) +
  geom_vline(xintercept = mean(pca19_df$PC8) + 2 * sd(pca19_df$PC8), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC8) - 2 * sd(pca19_df$PC8), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC8) + sd(pca19_df$PC8), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC8) - sd(pca19_df$PC8), linetype = "dashed") +  
  geom_beeswarm(size=2, spacing =1, alpha = .7, corral="wrap", corral.width = 1) +
  scale_color_viridis_c() +
  xlab(paste0("PC8: ",round(percentVar[8] * 100),"% variance")) +
  theme_minimal() +
  scale_y_discrete(limits=rev) +
  labs(color = expression(degree*Brix), y = "time (hours)") 
```
...
```{r all_pca19_df_ava}
pca1_19p_ava <- ggplot(pca19_df, aes(x = PC1, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(pca19_df$PC1) + 2 * sd(pca19_df$PC1), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC1) - 2 * sd(pca19_df$PC1), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC1) + sd(pca19_df$PC1), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC1) - sd(pca19_df$PC1), linetype = "dashed") + 
...
pca2_19p_ava <- ggplot(pca19_df, aes(x = PC2, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(pca19_df$PC2) + 2 * sd(pca19_df$PC2), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC2) - 2 * sd(pca19_df$PC2), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC2) + sd(pca19_df$PC2), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC2) - sd(pca19_df$PC2), linetype = "dashed") + 
...
pca3_19p_ava <- ggplot(pca19_df, aes(x = PC3, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(pca19_df$PC3) + 2 * sd(pca19_df$PC3), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC3) - 2 * sd(pca19_df$PC3), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC3) + sd(pca19_df$PC3), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC3) - sd(pca19_df$PC3), linetype = "dashed") + 
...
pca4_19p_ava <- ggplot(pca19_df, aes(x = PC4, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(pca19_df$PC4) + 2 * sd(pca19_df$PC4), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC4) - 2 * sd(pca19_df$PC4), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC4) + sd(pca19_df$PC4), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC4) - sd(pca19_df$PC4), linetype = "dashed") + 
...
pca5_19p_ava <- ggplot(pca19_df, aes(x = PC5, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(pca19_df$PC5) + 2 * sd(pca19_df$PC5), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC5) - 2 * sd(pca19_df$PC5), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC5) + sd(pca19_df$PC5), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC5) - sd(pca19_df$PC5), linetype = "dashed") + 
...
pca6_19p_ava <- ggplot(pca19_df, aes(x = PC6, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(pca19_df$PC6) + 2 * sd(pca19_df$PC6), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC6) - 2 * sd(pca19_df$PC6), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC6) + sd(pca19_df$PC6), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC6) - sd(pca19_df$PC6), linetype = "dashed") + 
...
pca7_19p_ava <- ggplot(pca19_df, aes(x = PC7, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(pca19_df$PC7) + 2 * sd(pca19_df$PC7), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC7) - 2 * sd(pca19_df$PC7), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC7) + sd(pca19_df$PC7), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC7) - sd(pca19_df$PC7), linetype = "dashed") + 
...
pca8_19p_ava <- ggplot(pca19_df, aes(x = PC8, y = ava_id, color=hr)) +
  geom_vline(xintercept = mean(pca19_df$PC8) + 2 * sd(pca19_df$PC8), linetype = "dotted") +
  geom_vline(xintercept = -mean(pca19_df$PC8) - 2 * sd(pca19_df$PC8), linetype = "dotted") +  
  geom_vline(xintercept = mean(pca19_df$PC8) + sd(pca19_df$PC8), linetype = "dashed") +
  geom_vline(xintercept = -mean(pca19_df$PC8) - sd(pca19_df$PC8), linetype = "dashed") + 
...
```{r pca1_thru_pca4_2019, fig.height= 3.7}

# combine relevant hr plts
pca1_pca4_19_hr <- ggarrange(pca1_19p, pca2_19p, pca3_19p, pca4_19p, ncol = 1, legend = "none",
                             common.legend = T, labels = c("A", "", ""))

# combine relevant ava plts
pca1_pca4_19_ava <- ggarrange(pca1_19p_ava, pca2_19p_ava, pca3_19p_ava, pca4_19p_ava, 
                              ncol = 1, legend = "none",
                              common.legend = T, labels = c("B", "", ""))
pdf("pca1-thru-pca4.pdf", width = 6.5, height = 8.5)
ggarrange(pca1_pca4_19_hr, pca1_pca4_19_ava, legend = "none")
dev.off()
```

```{r pca5_thru_pca8_2019, fig.height= 4}

# combine relevant ava plts
pca5_pca8_19_ava <- ggarrange(pca5_19p_ava, pca6_19p_ava, pca7_19p_ava, pca8_19p_ava, 
                              ncol = 1, legend = "none", common.legend = T, 
                              labels = c("B", "", "", ""))

# combine relevant hr plts
pca5_pca8_19_hr <- ggarrange(pca5_19p, pca6_19p, pca7_19p, pca8_19p, ncol = 1, 
                             legend = "none", common.legend = T, 
                             labels = c("A", "", "", ""))
pdf("pca4-thru-pca8.pdf", width = 6.5, height = 8.5)
ggarrange(pca5_pca8_19_hr, pca5_pca8_19_ava, legend = "none")
dev.off()
```
...
```{r tsne19}
library(Rtsne)
tsne19 <- Rtsne(t(assay(vsd19)), check_duplicates=FALSE, pca=TRUE, perplexity=5, theta=0.5, dims=2)
dtsne19 <- as.data.frame(tsne19$Y)
dtsne19$sample <- colnames(assay(vsd19))
dtsne19 <- left_join(dtsne19, design19)
tsne19_perplexity5 <- ggplot(dtsne19, aes(x = V1, y = V2, color = hr)) +
  geom_point()+
  theme_minimal() +
  ggtitle("perplexity 5") +
  scale_color_brewer(palette = "Paired", name = "hour")

tsne19 <- Rtsne(t(assay(vsd19)), check_duplicates=FALSE, pca=TRUE, perplexity=6, theta=0.5, dims=2)
dtsne19 <- as.data.frame(tsne19$Y)
dtsne19$sample <- colnames(assay(vsd19))
dtsne19 <- left_join(dtsne19, design19)
tsne19_perplexity6 <- ggplot(dtsne19, aes(x = V1, y = V2, color = hr)) +
  geom_point()+
  theme_minimal() +
  ggtitle("perplexity 6") +
  scale_color_brewer(palette = "Paired", name = "hour")
  
tsne19 <- Rtsne(t(assay(vsd19)), check_duplicates=FALSE, pca=TRUE, perplexity=20, theta=0.5, dims=2)
dtsne19 <- as.data.frame(tsne19$Y)
dtsne19$sample <- colnames(assay(vsd19))
dtsne19 <- left_join(dtsne19, design19)
tsne19_perplexity20 <- ggplot(dtsne19, aes(x = V1, y = V2, color = hr)) +
  geom_point()+
  theme_minimal() +
...
```{r pca_hyp}
# check that colnames in counts match rownames in info
colnames(counts) <- gsub("GSM......._", "", colnames(counts))
all.equal(colnames(counts), dc_hyp$sample)

designhyp <- dc_hyp %>%
  dplyr::select(sample, time, deletion)

dds_hyp <- DESeqDataSetFromMatrix(countData = counts, 
                                colData = designhyp, 
                                design = ~ deletion + time)
vsd_hyp <-  vst(dds_hyp)
# perform a PCA on the data in assay(x) for the selected genes
pca_hyp <- prcomp(t(assay(vsd_hyp)))

# the contribution to the total variance for each component
percentVar_hyp <- pca_hyp$sdev^2 / sum(pca_hyp$sdev^2)

pca_hyp_df <- pca_hyp$x %>%
  as.data.frame() %>%
  rownames_to_column("sample") %>%
  left_join(designhyp)

# filter to wildtype like plot in main text
pca_hyp_df <- pca_hyp_df %>% filter(deletion == "HAP1")
pca_hyp <- ggplot(pca_hyp_df, aes(x=PC1, y=time, color=time)) + 
  geom_point(size = 2) + 
  xlab(paste0("PC1: ",round(percentVar_hyp[1] * 100),"% variance")) +
  theme_minimal() +
  ylab("time (minutes)") +
  scale_y_discrete(limits=rev) +
  scale_color_manual(values = c("#CE2220", "#E67F33", "#D0B541", "#7EB875", "#57A2AC", "#4E78C4", "#824D99", "#B997C7"))
...
pdf(file = "tsne_pca_revision.pdf", height = 6, width = 6)
ggarrange(ggarrange(
  tsne19_perplexity5, tsne19_perplexity6, tsne19_perplexity20, common.legend = T, legend = "bottom", nrow = 3),
  ggarrange(pca_hyp, ggarrange(pca1_19p2, legend = "none"), pca_brix19, legend = "bottom", heights = c(0.5, 0.3, 0.6), nrow = 3),
  widths = c(0.4, 0.6))
dev.off()
...
```{r tsne_hyp}
library(Rtsne)
tsne19 <- Rtsne(t(assay(vsd19)), 
                check_duplicates=FALSE, pca=TRUE, perplexity=6, theta=0.5, dims=2)
dtsne19 <- as.data.frame(tsne19$Y)
dtsne19$sample <- colnames(assay(vsd19))
dtsne19 <- left_join(dtsne19, design19)
tsne_hr19 <- ggplot(dtsne19, aes(x = V1, y = V2, color = hr)) +
  geom_point()+
  theme_minimal() 

tsne_brix19 <- ggplot(dtsne19, aes(x = V1, y = V2, color = brix)) +
  geom_point()+
  theme_minimal() +
  scale_color_viridis_c()

tsne_ava19 <- ggplot(dtsne19, aes(x = V1, y = V2, color = ava)) +
  geom_point()+
  theme_minimal() +
  scale_color_manual(values = c("OR"=or,  "AV" = av, "SNC" = snc, "RRV" = rrv, "CRN" = crn,
                                "AS" = as, "SMV" = smv, "SRH" = srh))
...
```{r DC1_DC219}
# dc19$hr <- factor(dc19$hr, levels = c(2, 6, 16, 64, 112))
# dm_hr19 <- ggplot(dc19, aes(x = DC1, y = DC2, color = hr)) +
#   geom_point()+
#   theme_minimal() 
# 
# dm_ava19 <- ggplot(dc19, aes(x = DC1, y = DC2, color = ava)) +
#   geom_point()+
#   theme_minimal() +
#   scale_color_manual(values = c("OR"=or,  "AV" = av, "SNC" = snc, "RRV" = rrv, "CRN" = crn,
#                                 "AS" = as, "SMV" = smv, "SRH" = srh))
...
#   theme_minimal() 
# 
# dm_brix19 <- ggplot(dc19, aes(x = DC1, y = DC2, color = brix)) +
#   geom_point()+
#   theme_minimal() +
#   scale_color_viridis_c()
```
...
```{r combine_plts}
# plts_hr19 <- ggarrange(dm_hr19, pca_hr19, tsne_hr19, nrow = 3, common.legend = T, legend = "bottom")
# plts_brix19 <- ggarrange(dm_brix19, pca_brix19, tsne_brix19, nrow = 3, common.legend = T, legend = "bottom")
# plts_ava19 <- ggarrange(dm_ava19, pca_ava19, tsne_ava19, nrow = 3, common.legend = T, legend = "bottom")
# 
# ggarrange(plts_hr19, plts_brix19, plts_ava19, ncol = 3)
```
...
```{r brix_upset19}
brix_list_19 <- list("DC1 induced" = filter(dc1_19, logFC > 0)$ORF,
                     "DC1 repressed" = filter(dc1_19, logFC < 0)$ORF,
                     "Brix induced" = filter(brix_19, logFC > 0)$ORF,
                     "Brix repressed" = filter(brix_19, logFC < 0)$ORF
)
UpSetR::upset(fromList(brix_list_19), nsets = 8, nintersects = 200, 
...
ggarrange(dc_top_diffex_plot(dc = dc19, dc_num = "DC2", dc_con = dc2_19, plot_by = "hr"),
          dc_top_diffex_plot(dc = dc19, dc_num = "DC3", dc_con = dc3_19, plot_by = "hr"),
          dc_top_diffex_plot(dc = dc19, dc_num = "DC4", dc_con = dc4_19, plot_by = "hr"),
          nrow = 3, labels = c("A", "B", "C"), common.legend = T, legend = "bottom")
```
...
pdf("dc5-dc8_top_genes_revision.pdf", width=6.5, height = 8.5)
ggarrange(dc_top_diffex_plot(dc = dc19, dc_num = "DC5", dc_con = dc5_19, plot_by = "ava"),
          dc_top_diffex_plot(dc = dc19, dc_num = "DC6", dc_con = dc6_19, plot_by = "ava"),
          dc_top_diffex_plot(dc = dc19, dc_num = "DC7", dc_con = dc7_19, plot_by = "ava"),
          dc_top_diffex_plot(dc = dc19, dc_num = "DC8", dc_con = dc8_19, plot_by = "ava"),
...
## 2017 data -- removed from manuscript
...
```{r pca17}
# check that colnames in counts match rownames in info
all.equal(colnames(counts17), dc17$sample)

design17 <- dc17 %>%
  dplyr::select(sample, ava_id, hr, brix, ava) %>%
  mutate(hr = ifelse(hr < 20, 16, hr)) %>%
  mutate(hr = ifelse(hr > 20 & hr < 46, 40, hr)) %>%
  mutate(hr = ifelse(hr > 47 & hr < 76, 64, hr)) %>%
  mutate(hr = ifelse(hr > 76 & hr < 109, 88, hr)) %>%
  mutate(hr = ifelse(hr > 110, 112, hr))

design17$hr <- factor(design17$hr, levels = c(16, 40, 64, 88, 112))

dds17 <- DESeqDataSetFromMatrix(countData = counts17, 
                                colData = design17, 
                                design = ~ ava_id + hr)
vsd17 <-  vst(dds17)
# perform a PCA on the data in assay(x) for the selected genes
pca17 <- prcomp(t(assay(vsd17)))

# the contribution to the total variance for each component
percentVar <- pca17$sdev^2 / sum(pca17$sdev^2)
...
pca_brix17 <- ggplot(pca17_df, aes(x=PC1, y=PC2, color=brix)) + 
  geom_point() + 
  xlab(paste0("PC1: ",round(percentVar[1] * 100),"% variance")) +
  ylab(paste0("PC2: ",round(percentVar[2] * 100),"% variance")) +
  theme_minimal() +
...
pca_hr17 <- ggplot(pca17_df, aes(x=PC1, y=PC2, color=hr)) + 
  geom_point() + 
  xlab(paste0("PC1: ",round(percentVar[1] * 100),"% variance")) +
  ylab(paste0("PC2: ",round(percentVar[2] * 100),"% variance")) +
  theme_minimal() 

pca_ava17 <- ggplot(pca17_df, aes(x=PC1, y=PC2, color=ava)) + 
  geom_point() + 
  xlab(paste0("PC1: ",round(percentVar[1] * 100),"% variance")) +
  ylab(paste0("PC2: ",round(percentVar[2] * 100),"% variance")) +
  theme_minimal() +
...
```{r tsne17}
library(Rtsne)
tsne17 <- Rtsne(t(assay(vsd17)), 
                check_duplicates=FALSE, pca=TRUE, perplexity=5, theta=0.5, dims=2)
dtsne17 <- as.data.frame(tsne17$Y)
dtsne17$sample <- colnames(assay(vsd17))
dtsne17 <- left_join(dtsne17, design17)
tsne_hr17 <- ggplot(dtsne17, aes(x = V1, y = V2, color = hr)) +
  geom_point()+
  theme_minimal()

tsne_brix17 <- ggplot(dtsne17, aes(x = V1, y = V2, color = brix)) +
  geom_point()+
  theme_minimal() +
  scale_color_viridis_c()


tsne_ava17 <- ggplot(dtsne17, aes(x = V1, y = V2, color = ava)) +
  geom_point()+
  theme_minimal() +
  scale_color_manual(values = c("OR"=or,  "AV" = av, "SNC" = snc, "RRV" = rrv, "CRN" = crn,
                                "AS" = as, "SMV" = smv, "SRH" = srh))
...
dc17_2 <- dc17 %>%
  ...
  mutate(hr = ifelse(hr > 76 & hr < 109, 88, hr)) %>%
  mutate(hr = ifelse(hr > 110, 112, hr))
dc17_2$hr <- factor(dc17_2$hr, levels = c(16, 40, 64, 88, 112))
dm_hr17 <- ggplot(dc17_2, aes(x = DC1, y = DC2, color = hr)) +
  geom_point()+
  theme_minimal() 

dm_ava17 <- ggplot(dc17_2, aes(x = DC1, y = DC2, color = ava)) +
  geom_point()+
  theme_minimal() +
  scale_color_manual(values = c("OR"=or,  "AV" = av, "SNC" = snc, "RRV" = rrv, "CRN" = crn,
                                "AS" = as, "SMV" = smv, "SRH" = srh))

dm_brix17 <- ggplot(dc17_2, aes(x = DC1, y = DC2, color = brix)) +
  geom_point()+
  theme_minimal() +
  scale_color_viridis_c()
```
...
```{r combine_plts17}
plts_hr17 <- ggarrange(dm_hr17, pca_hr17, tsne_hr17, nrow = 3, common.legend = T, legend = "bottom")
plts_brix17 <- ggarrange(dm_brix17, pca_brix17, tsne_brix17, nrow = 3, common.legend = T, legend = "bottom")
plts_ava17 <- ggarrange(dm_ava17, pca_ava17, tsne_ava17, nrow = 3, common.legend = T, legend = "bottom")

ggarrange(plts_hr17, plts_brix17, plts_ava17, ncol = 3)
```