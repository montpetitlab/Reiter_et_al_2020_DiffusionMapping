...
However, synchronicity is difficult to maintain across groups of cell populations exposed to different treatments.
...
An additional challenge is the identification of differences between groups that are separate from or in excess to the dominant and ubiquitous global shift in gene expression.
Structure in gene expression exists at different scales (CITE PHATE), but dominant shifts experienced by all groups may overpower weaker but biologically important signals.
...
Dimensionality reduction algorithms like principal component analysis and t-distributed stochastic neighbor embedding (tSNE) are commonly applied to sequencing data to identify sources of variation therein [@way2020compressing] (CITE PHATE).
...
Here, we use diffusion maps to analyze bulk time-series RNA-sequencing of global shifts of *Saccharomyces cerevisiae* metabolism during wine fermentation from different vineyard sites and hypoxia of mutants.
...
Then, we apply differential expression along each diffusion component to determine the genes responsible for separation among samples.
...
<!-- Unlike brix measurements, latent variables-->
...
While *S. cerevsisiae* gene expression during fermentation is consistent across diverse fermentations conditions (CITE BRIX PAPER), the chemical composition of grape must and wine is variable however wines from the same vineyard site often have similar outcomes across multiple years (e.g. vintages) [@cantu2020investigating] (CITE).
This phenomenon is referred to as *terroir* -- the environmental and anthropomorphic factors contribute to the outcome of wine. 
...
We performed 60 inoculated primary fermentations of genetically similar Pinot noir grapes grown in 15 vineyards in California and Oregon over two vintages (**Figure \@ref(fig:map)**). 
...
When we apply diffusion mapping to wine fermentation, we find that diffusion component 1 captures the dominant metabolic transition of *S. cerevisiae* during fermentation and re-orders samples according to brix which reflects stage of fermentation (**Figure \@ref(fig:dc1)**).
...
Log~2~ fold change values are highly correlated between both methods of differential expression (**Figure \@ref(fig:dc1)C, Figure \@ref(fig:upsetgrid)**), indicating that the primary diffusion component captures the dominant global shift in gene expression during fermentation.
...
# by_hour_67 <- ggplot(dc19, aes(x = DC6, y = DC8, color = factor(hr, levels = c("0", "2", "6", "16", "64", "112")), label = ava)) +
#   geom_point(alpha = .8) +
#   scale_color_brewer(palette = "Paired", name = "hour") + 
#   theme_minimal() +
#   theme(plot.title = element_text(hjust = 0.5)) 
# 
# by_ava_67 <- ggplot(dc19, aes(x = DC6, y = DC8, color = ava, label = ava)) +
#   geom_point() +
#   theme_minimal() +
#   scale_color_manual(values = c("OR"=or,  "AV" = av, "SNC" = snc, "RRV" = rrv, "CRN" = crn,
#                                 "AS" = as, "SMV" = smv, "SRH" = srh)) + 
...
```{r v2019dc4dc6, fig.height = 3, fig.cap = "Diffusion component 8 and diffusion component 10 capture variation in late fermentation. While early fermentation samples cluster at the origin, late fermentation samples, especially those from regions AS, AV, and SNC, form distinct clusters."}
by_hour <- ggplot(dc19, aes(x = DC4, y = DC6, color = factor(hr, levels = c("0", "2", "6", "16", "64", "112")), label = ava_id)) +
  geom_point() +
  scale_color_brewer(palette = "Paired", name = "hour") + 
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) 

by_ava <- ggplot(dc19, aes(x = DC4, y = DC6, color = ava, label = ava_id)) +
  geom_point() +
  scale_color_manual(values = c("OR"=or,  "AV" = av, "SNC" = snc, "RRV" = rrv, "CRN" = crn,
                                  "AS" = as, "SMV" = smv, "SRH" = srh)) + 
  theme_minimal() +
...
# add WT
scat1 + 
  geom_point(data=subset(dc_hyp, deletion == "HAP1"), aes(x = DC1, y = DC2), shape = 3, size = 2)
```
